...
class Bottom_up():
    ...
    def generate_new_train_label(self,step, idx1, idx2,u_feas,label,num_to_merge,dists,flag,view_to_images,krnn_set):
        ...
        for i in range(len(b)):
            ...
            idx2=d[i][1]
            same_c=[]
            for k in view_to_images[label2]:
                if dists[idx1,k]==10000:
                    c_dist.append(dists[k,idx1])    
                else:
                    c_dist.append(dists[idx1,k])
                    
            for k in view_to_images[label1]:
                if dists[idx2,k]==10000:
                    c_dist.append(dists[k,idx2])
                else:
                    c_dist.append(dists[idx2,k])
