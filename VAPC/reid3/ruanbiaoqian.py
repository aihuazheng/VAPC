...
class Bottom_up():
    ...
    def train(self, train_data, step,common_kcrnn, loss, dropout=0.5):
        ...
        for epoch in range(epochs):#epochs
            ...
            trainer.train(epoch, dataloader, optimizer, common_kcrnn,print_freq=max(5, len(dataloader) // 30 * 10))
    ...
    def get_new_train_data(self, step, labels, nums_to_merge, size_penalty):
        ...
        if self.rank1>self.bestrank1:
            ...
            'state_dict': self.model.module.state_dict(),
            'step': 1,
            }, fpath=osp.join('logs', 'vottominitnobanlance.pth.tar'))
            np.savetxt("vottominitnobanlance.txt",labels)   
        """
    ...
    #    for j in label_to_images[i]: 
        ...
        common_kcrnn=OrderedDict()    
        for j in range(len(labels)):
            id1 = [i for i,x in enumerate(labels) if x==labels[j]]
            common_kcrnn[j]=id1
        """
