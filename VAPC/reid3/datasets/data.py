...
class Data(object):
    ...
    def preprocess(self, images_dir, path, relabel=True, fakelabel=False):
        ...
        for path_here in paths:
            if 'cuhk03' in images_dir:
                ...
                fpaths1 = sorted(glob(osp.join('/home/sunxia/data/VeRi776/VeRi/VeRiTrain/', '1', '*.jpg')))
                
                fpaths2 = sorted(glob(osp.join('/home/sunxia/data/VeRi776/VeRi/VeRiTrain/', '2', '*.jpg')))
                fpaths3 = sorted(glob(osp.join('/home/sunxia/data/VeRi776/VeRi/VeRiTrain/', '3', '*.jpg')))
                fpaths4 = sorted(glob(osp.join('/home/sunxia/data/VeRi776/VeRi/VeRiTrain/', '4', '*.jpg')))
                fpaths5 = sorted(glob(osp.join('/home/sunxia/data/VeRi776/VeRi/VeRiTrain/', '5', '*.jpg')))
            ...
            #if 'VeriTrain' in images_dir:
                #print(images_dir, "VeriTrain", ret)
            #    for i in ret:
            #        tmp.append(i[1])
            #    np.savetxt("gd.txt",tmp,fmt="%d")
    ...
    def load(self):
        ...
        self.target_train, self.num_target_ids = self.preprocess('/home/sunxia/data/VeRi776/VeRi/VeriTrain', 'VeriTrain',fakelabel=True)
        
        """
        with open("/home/sunxia/data/VeRi/color.json",'r', encoding='UTF-8') as f:
            color_dic = json.load(f)
        color=[]
        for i in self.target_train:
            #print("i[0]",i[0])
            color.append(int(color_dic[i[0]]))
            #print("color_dic[i[0]]",color_dic[i[0]])
        print(color)
        np.savetxt("color.txt",color,fmt="%d")
        print(color)
        """
        #print("self.target_train",self.target_train)                                               
