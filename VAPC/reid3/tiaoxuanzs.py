...
class Bottom_up():
    def __init__(self, model_name, batch_size, num_classes,view_label,dataset, u_data, save_path, embeding_fea_size=1024,
        ...
        self.u_data = u_data
        self.u_label = np.loadtxt("/DATA/sunxia/beifeng/homebak/sunxia/Bottom/gd.txt")#np.array([label for _, label, _, _ in u_data])
        ...
        checkpoint = load_checkpoint("/DATA/sunxia/beifeng/homebak/sunxia/Bottom/logs/bestcheckpoint038432.pth.tar") 
    ...
    def train(self, train_data, step, loss, dropout=0.5):
        ...
        def adjust_lr(epoch, step_size):
            lr = 0.01#init_lr / (10 ** (epoch // step_size))
            print("lr",lr)
    ...
    def evaluate(self,query, gallery, output_feature):
        ...
        query_loader = DataLoader(
            Preprocessor(query,root=self.dataset.target_images_dir, transform=test_transformer),
            batch_size=64, num_workers=self.data_workers,
            shuffle=False, pin_memory=True)
        gallery_loader = DataLoader(
            Preprocessor(gallery,root=self.dataset.target_images_dir, transform=test_transformer),
            batch_size=64, num_workers=self.data_workers,
            shuffle=False, pin_memory=True)
    ...
    def get_new_train_data(self, step, labels, nums_to_merge, size_penalty):
        ...
        """
        if self.rank1>self.bestrank1:
            print("save best checkpoint")
            self.bestrank1=self.rank1
            save_checkpoint({
            'state_dict': self.model.module.state_dict(),
            'step': 1,
            }, fpath=osp.join('logs', 'notianxiaozs.pth.tar'))
        ...
        for v in range(0,1):
            ...
            s_noise=[]
            s_cluster=[]
            rerank_dist,oridist = re_ranking(0, u_feas, step,flag=0)
            ...
            if v ==-2:
                ...
                for k in rec_noise_index:
                    ...
                    if tmp in rec_noise_index:
                        for j in initial_rank[k,1:8]:
                            #print("j",j)
                            
                            #if j in tmp_rec:
                            #    continue     
                            if j in rec_noise_index:
                                #for k in label[initial_rank[rec_noise_index[k],1:10]]:
                                #print("label[initial_rank[np.where(rec_noise==j)[0],1:10]]",label[initial_rank[np.where(rec_noise==j)[0],1:10]])
                                #print("initial_rank[j,1:10]",initial_rank[j,1:10])
                                if k in initial_rank[j,1:4]:  
                                    s_noise.append((k,j))
                                    tmp_rec.append(j)
                                    tmp_rec.append(k)
                                else:
            ...
            np.savetxt("labelsnotp"+str(step)+".txt",labels)
