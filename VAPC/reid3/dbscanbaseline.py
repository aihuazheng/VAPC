...
class Bottom_up():
    ...
    def get_dataloader(self, dataset, training=False):
        ...
        data_loader = DataLoader(
            Preprocessor(dataset, root="/home/sunxia/VeRi/VeRiTrain/",
    ...
    def get_new_train_data(self, step, labels, nums_to_merge, size_penalty):
        ...
        if self.rank1>self.bestrank1:
            ...
            'state_dict': self.model.module.state_dict(),
            'step': 1,
            }, fpath=osp.join('logs', 'dbscanbanlance2.pth.tar'))
            np.savetxt("dbscanbanlance2.txt",labels)   
        """
        ...
        s_noise=[]
        s_cluster=[]
        rerank_dist= re_ranking(0, u_feas, step=2,flag=1)
        print("rerank_dist",rerank_dist.shape)
        tri_mat = np.triu(rerank_dist, 1) # tri_mat.dim=2
        tri_mat = tri_mat[np.nonzero(tri_mat)] # tri_mat.dim=1
        tri_mat = np.sort(tri_mat,axis=None)
        top_num = np.round(0.5e-3*tri_mat.size).astype(int)
        ...
        labels = self.generate_new_train_label(step,idx1, idx2,u_feas,labels,nums_to_merge,dists,flag,label_to_images,self.krnn_set)
        """
        unique_label = np.sort(np.unique(np.array(label)))
        j=0
        for i in range(len(unique_label)):
            ...
            label = [j if x == label_now else x for x in label]
            j+=1
        #print("metrics.adjusted_rand_score(labels_true, labels_pred)",metrics.adjusted_rand_score(self.u_label, label))
        print("metrics.adjusted_mutual_info_score(labels_true, labels_pred)",metrics.adjusted_mutual_info_score(self.u_label, label))
        #print("v_measure_score",metrics.v_measure_score(self.u_label, label)) 
        new_train_data, label_to_images = self.generate_new_train_data(label)
